...
重写后服务端日志中旧命名空间相关异常由 1435 次降为 **0**。
...
预设箱里的普通 `SANDSTONE` 是方块皮肤占位符:发放套装时会按该堆原数量替换为玩家
当前选择的方块皮肤。`CUT_SANDSTONE` 等其他砂岩变体仍是普通物品;箱子原内容不会被
修改。皮肤服务暂时不可用时会保留原 `SANDSTONE`,确保整套预设仍可正常发放。
